...
class PostgresIngress(BaseIngress):
    ...
    async def _init_pool(self) -> None:
        """Initializes the asyncpg pool and ensures the parameter table exists."""
        if not self.pool:
            self.pool = await asyncpg.create_pool(dsn=self.dsn, **self.kwargs)
            logger.info(f"PostgresIngress connected to {self.table_name}")

            # Ensure the table exists for convenience
    ...
    async def run(self, ingress_queue: queue.Queue) -> None:
        ...
        while True:
            try:
                ...
                logger.error(f"Error polling PostgresIngress table {self.table_name}: {e}")
