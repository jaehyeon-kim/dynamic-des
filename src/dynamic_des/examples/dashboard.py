...
# 1. Initialize Admin Connector
admin = KafkaAdminConnector(bootstrap_servers="localhost:9092", max_tasks=200)

# 2. State Management
ui_state = {
    "selected_resource": None,
    "capacity": 0,  # Current staffed level
    "max_capacity": 10,  # Physical ceiling
    "in_use": 0,
    "queue": 0,
    "util": 0.0,
    "avg": "0.00s",
...
def process_data(log_element):
    ...
    if sim_id in state:
        for service_name, tasks in state[sim_id].items():
            for task_id, timestamps in tasks.items():
...
async def index():
    ...
    with ui.column().classes("w-full max-w-7xl mx-auto p-6 gap-6"):
        ...
        with ui.row().classes("w-full justify-between items-stretch"):
            ...
            with ui.card().classes(
                ...
                util_ring = (
                    ui.circular_progress(value=0, max=100, show_value=True, size="70px")
                    .props("thickness=0.2")
                    .bind_value_from(ui_state, "util")
                )

        # --- MIDDLE ROW: KPIs, CHART, FEED (Aligned with Top Row) ---
        with ui.row().classes("w-full gap-4 items-stretch"):
            ...
            with ui.column().classes("w-56 gap-4"):
                ...
                def k_card(t, k, bg):
                    with ui.card().classes(
                        ...
                        ui.label(t).classes("text-xs font-bold text-gray-500 uppercase")
            ...
            with ui.card().classes("flex-grow p-4 shadow-sm h-[344px]"):
                ui.label("REAL-TIME TRENDS").classes("text-xs font-bold text-gray-400")
    ...
    ui.timer(2.0, lambda: process_data(feed))
...
ui.run(title="Real-time Capacity Simulator", port=8080, show=False)
